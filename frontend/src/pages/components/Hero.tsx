...
import {NegotiationPackageType} from "@p2p-library/connection/negotiationManager.ts";

const Hero: FC = () => {
  const [callId, setCallId] = React.useState<string>('');
  const navigate = useNavigate();

  const onCall = useCallback((data: { np: NegotiationPackageType, targetPeerId: string }) => {
    navigate('/callinterface', {state: data})
  }, [])

  useEffect(() => {
    connector.eventEmitter.on("onCall", onCall)

    return () => {
      connector.eventEmitter.off("onCall", onCall)
    }
  }, []);

  function onClickCall() {
    navigate('/callinterface', {state: {np: undefined, targetPeerId: callId}});
  }
          ...
          >
            ...
            >
              ...
              >
                ...
                />
                <IconButton color="success" onClick={onClickCall}>
                  <CallIcon/>
                </IconButton>
              </Box>
            </Stack>
